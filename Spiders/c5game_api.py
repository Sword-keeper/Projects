...
def for_item(item_name, do_function=do_nothing):
    ...
    return scan_page(url, do_function)
...
def for_all_items(do_function=do_nothing, use_multi_process=True):
    ...
    params = [{'url': url, 'function': do_function} for url in urls]
    if use_multi_process:
        multi_process(scan_url, params, processes=8)
    else:
        for param in params:
            scan_url(param)
...
def scan_url(param):
    url = param.get('url')
    do_function = param.get('function')
    ...
    if html:
        ...
        if last_page:
            ...
            for x in range(1, max_page + 1):
                url = base + str(x)
                scan_page(url, do_function)


def scan_page(url, do_function=do_nothing):
    html = get_page(url)
    if html:
        soup = BeautifulSoup(html, 'lxml')
...
def scan_item(url, do_function=do_nothing, price=None, purchasing=False):
    html = get_page(url)
    if html:
        soup = BeautifulSoup(html, 'lxml')
