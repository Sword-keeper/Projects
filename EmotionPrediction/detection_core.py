...
def quick_detection(path, detector=None):
    if not detector:
    ...
    for p in detector.run_detection(path_list):
        result = p[1]
        if result:
            ret = [(x['class'], x['score']) for x in result]
    return ret
...
if __name__ == '__main__':
    ...
    # abstract level
    abstract = max([score for x, score in res])
    ...
    distribution = {}
    sum = 0
    for dtc_class, score in res:
        for emotion in emotions:
            dis = model.wmdistance(dtc_class, emotion)
            val = score / dis
            sum += val
            if emotion in distribution:
                ...
                distribution[emotion] = val
    for emotion in emotions:
        distribution[emotion] = distribution[emotion] / sum
